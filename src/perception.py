...
# body of revolution standing on the belt, i.e. genuinely round, not a lump whose
# convex hull merely smooths into a circle. Measured (docs/probe-models.md):
# upright can 0.0002, ball 0.0236 | real Мешок frame 0.0702. 0.04 sits between,
# ~1.7x above the ball and ~1.8x below the bag. Only silhouettes that ALREADY read
# round (K > 0.8) and thick ever reach this test, so Шлем (K=0.74) never does —
# which matters, because the helmet dome is MORE axially symmetric (0.0551) than
# the bag and would otherwise be pulled into D against its reference B.
