...
> а на реальную линию мы закладываем три встречной конфигурации 3A. Ответ получен
> постройкой и замером четырёх конфигураций, а не выбором; само решение и его
> цена — в [`rig_decision.md`](rig_decision.md). Здесь сведены **все** числа развилки, у
...
| **Закладываем в реальную линию** | **3A: верх + две ВСТРЕЧНЫЕ боковые** (решение 26.07, разбор и цена — [`rig_decision.md`](rig_decision.md)) | Выбрано по критериям, объявленным до прогонов: третья головка мисроутов не добавляет по продовому правилу (0,0 против 0,7 у двух на 165 позах) и даёт +10 п.п. допуска; при ослепшей верхней головке две встречные боковые дают 6,2 мм против 48,6 мм у одной, потому что поперечный размер восстанавливается брекетингом, а не обзором. Кандидат «головка вдоль ленты» (3B) отклонён числом — ни одной клетки в свою пользу ни при одном из пяти правил слияния. **Условие, без которого решение недействительно: преимущество третьей головки целиком калибровочное** (+14 / +10 / +2 п.п. по мере деградации), и по худшему столбцу две головки бьют 3A. Процедура калибровки на смену — условие эксплуатации этой стойки, а не опция. |

**Оговорка к строке «реальная линия», и её надо произносить самим.** До 26.07 эта
строка читалась «2 минимум, 3 — покрытие дальнего фланга», и её обоснование
приводится ниже целиком, потому что оно осталось верным — изменилось только число
головок, а не логика. Обоснование — **доступность** (выпадение возврата глубины на тёмной, бликующей, прозрачной упаковке), а не точность. Наш бесшумный Gazebo этого явления не содержит вообще, поэтому довод расчётный: паспорт сенсора D435-класса + отрасль ([SICK VMS4200/5200 — две головки, Cubiqnet — три плоскости](industry_comparison.md)). **Но доступность придётся ещё построить, и это надо сказать раньше, чем спросят: отгружаемый контракт слияния её не даёт ни при двух головках, ни при трёх.** Боковой кадр входит в измерение только внутри объёма, который верхняя головка уже назначила товару (кроп по `measurement.position_m` и `measurement.dims_mm`, `src/perception_node.py:96-115`), а публикация в ячейке ровно одна — внутри обработчика верхнего кадра (`src/perception_node.py:163`). Нет верхнего кадра — нет измерения при любом числе боковых (§8 T3). Редундантность у нас — **проектное требование к следующей версии слияния**, а не уже измеренное свойство.
