...
- Геометрия обоих миров покрыта XML-контрактными тестами. На момент выпуска
  чертежа `scripts/check_sdf.sh` проверяет 11 товарных моделей и базовый
  `cell.sdf`; добавление финального мира в этот быстрый скрипт выделено
  отдельным infra-шагом, чтобы отсутствие строки проверки не маскировать.
- Полный body-scored census на этой компоновке: `33/33 ×3`.
