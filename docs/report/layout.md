...
## Сверка с официальной схемой участка (QA 18.07)

Организаторы на QA-сессии показали официальную схему рабочей зоны
(`img/official_layout.png`) и подтвердили ряд свобод компоновки. Ниже — честное
сопоставление нашей симуляции с этой схемой.

![Официальная схема рабочей зоны](img/official_layout.png)

| Параметр | Официальная схема | Наша симуляция | Статус |
|---|---|---|---|
| Ширина подающей ленты | 500 мм | 500 мм | ✅ совпадает |
| Габарит рабочей зоны | ~7000 × 6000 мм | 6000 × 3100 мм | ✅ вписаны в участок |
| Зоны C/D | ролл-кейдж 1200×800×800 мм | клетка 1600×800, стенки 800 мм | ≈ близко (кейдж длиннее у нас) |
| Топология A→B/C/D | B дальше на конвейер, C/D в стороны | B едет прямо, C/D шиберами в стороны | ✅ совпадает |
| Высота поверхности ленты | 700 мм | 400 мм | ⚠️ упрощение (см. ниже) |
| Воронка-расширение после подачи | есть | нет (дивёрт на прямой ленте) | ⚠️ осознанно опущено |
| Зона B | конвейерная линия 500×700 | патч «едет прямо» | ⚠️ упрощение вывода |

**Подтверждённые на QA свободы компоновки (18.07):** накопитель не обязателен;
товар можно перехватывать **до** накопителя (наш дивёрт в движении легитимен);
накопитель можно менять как угодно; длину конвейера можно продлевать; конвейер
закреплён по положению. Наш подход (перенаправление в движении без накопителя и
захвата) организаторами прямо разрешён.

**Почему мы НЕ перестраиваем геометрию под мм-в-мм.** Расхождения (высота ленты
400 vs 700 мм, отсутствие воронки, B как патч) — это упрощения симуляции в
пределах разрешённого участка, а не нарушения правил. Приведение геометрии к
схеме мм-в-мм потребовало бы разморозки стабильного ядра `v0.5-stable-stream`
(census 33/33 ×3, полная потоковая матрица) за две недели до сдачи и повторной
валидации всего контура — риск, не оправданный маргинальным приростом точности
макета. Высота ленты и форма вывода не влияют на проверяемые правила
классификации и маршрутизации; они честно вынесены в ограничения
(`physics_assumptions.md`). Схема фиксирует референс, к которому мы приводим
физический проект перед изготовлением (STEP-сборка, п. «Доступ для обслуживания»).
...
- `scripts/smoke_estop_stream.sh` проверяет остановку занятой ячейки: лента и
  ...
  В кандидате этапа 26 тот же сценарий после ручной очистки ждёт измеренную
  парковку до soft-start; headless Fortress run `29591607813` подтвердил этот
  путь, ресурсный и полный merge-gate ещё открыты.
