...
|---|---:|---:|---|---|
| Мульти-seed census (**22.07, актуальное ядро depth-scale** `29e7f52`), seeds 0–4, 11 моделей × 3 ориентации | 5/5 census run | **163/165** routed (33/32/33/33/32) | **2 классификационных промаха** (execution 0), оба околопороговые по K: Шлем oi=1 seed1 (`324x298x284 мм K=0.80`) и Мешок oi=2 seed4 (`195x177x174 мм K=0.80`) → D у порога 0.8; в остальных четырёх seed'ах те же ячейки проходят | `runs/a2sweep_20260722_*_seed{0..4}` |
...
| Мульти-seed census (20.07, ядро ДО E-stop `c12ba2b`), seeds 0–4 | 5/5 census run | **164/165** routed | 1 промах: Мешок oi=2 seed4 (K=0.80→D, порог 0.8004); execution 0; **Pouf 15/15** | `runs/baseline_main_seed0`, `runs/sweep_main_seed{1,2,3,4}` |
...
Актуальная заявка — **163/165 на текущем ядре** (22.07, depth-scale, `29e7f52`,
распределение `33/32/33/33/32`). Мульти-seed по пяти seed’ам — величина
**run-to-run, а не константа**, и по ревизиям она шла так: 165/165 (19.07, ядро ДО
фикса K, `fa4d30e`), 164/165 (20.07, ядро до E-stop, `c12ba2b`), 162/165 (21.07,
ядро С E-stop, `bf891af`), **163/165 (22.07, depth-scale, `29e7f52`)**. Диапазон
**162–165/165** описывает историю ревизий, а не текущую заявку: складывать его с
актуальным числом в один знаменатель нельзя. Инвариант через все четыре прогона: **execution
