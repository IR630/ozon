...
Два census слитого состояния после сведения прозрачных бортиков, 3D body-OBB и
touching-perception дали `33/33 + 33/33`. Ограничение честности: seed фиксирует
сцену и ориентации, но контактная физика Gazebo имеет небольшой шум; поэтому
метрика принимается только по повторным прогонам с сохранённым triage.
