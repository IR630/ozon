...
> (17.07.2026, этапы 21–25 закрыты). Поверх заморозки принят один пакет —
> feedback-gated E-stop recovery (`int/estop-engaged-recovery`, влит 21.07):
> physical occupied gate закрыт run `29591607813`. Источники обоснований —
...
|---|---|---|---|
| `ros_gz_bridge` | топики Gazebo | ROS 2 camera, `/clock`, C/D `JointState`; команды обратно в Gazebo | Единый мост из `sim/bridge.yaml`; все ноды работают с `use_sim_time`; в кандидате Stage 26 позиционная ячейка не стартует без свежего подтверждения обоих углов |
...
| `controller_node` | `ItemClassification`, `/infeed/fed`, `/emergency_stop`, `/diverter_{c,d}/joint_state` | `/conveyor/cmd_vel`, `/pusher_{c,d}/cmd` | Перепланирует таймер каждого товара по свежей позиции, допускает не более одного физического действия на ID; кандидат Stage 26 удерживает измеренный угол только при свежей post-command обратной связи, а reset ждёт подтверждённую парковку |
...
Непрерывную перепись `33/33` даёт более поздний прогон уже на текущем ядре
(`runs/baseline_main_seed0`, `experiments.md` 2026-07-20) — он же снимает
оговорку о том, что прежние числа получены до фикса K. Мульти-seed по пяти
seed-ам — `162–165/165` (величина run-to-run; на ядре с E-stop — 162/165), все
промахи — околопороговые по K товары (Шлем, Мешок у 0.8), execution 0, разобраны там же.
