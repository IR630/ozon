...
sustained-результат отгружаемой ДВУХкамерной стойки — **9 товаров/мин** (30.07,
`runs/chain2_20260730_024359/throughput`); одноголовая стойка на той же матрице дала 9 (28.07)
и **10 товаров/мин** 17.07 (`run_stream_suite.sh 0 0 2`, 33/33; p95-медленный
такт 4); пиковый короткий 3-товарный поток даёт 18, но за sustained не выдаётся. Камера→решение —
медиана **0,042 с** на полной матрице отгружаемой двухкамерной стойки (30.07, n=33,
p95 0,088; на одноголовой стойке 17.07 было 0,030 с, на 3-товарных прогонах 0,094);
решение→команда
...
истине. Multi-seed перепись отгружаемой ДВУХкамерной стойки (24.07, seeds 0–4) —
`163/165`; одноголовым fallback-путём на depth-scale базе — те же `163/165`
ячейка в ячейку. Оба промаха околопороговые по K (Шлем, Мешок у 0.8),
execution 0.
Она выполнена fallback-путём камерной исследовательской ветки. Исторический
run-to-run диапазон прежних ревизий — `162–165/165`; точный последующий `main`
