...
> является кандидатом `int/estop-engaged-recovery` до физического gate. Источники обоснований —
...
|---|---|---|---|
| `ros_gz_bridge` | топики Gazebo | ROS 2 camera, `/clock`, C/D `JointState`; команды обратно в Gazebo | Единый мост из `sim/bridge.yaml`; все ноды работают с `use_sim_time`; позиционная ячейка не стартует без обоих углов |
...
| `controller_node` | `ItemClassification`, `/infeed/fed`, `/emergency_stop`, `/diverter_{c,d}/joint_state` | `/conveyor/cmd_vel`, `/pusher_{c,d}/cmd` | Перепланирует таймер каждого товара по свежей позиции, допускает не более одного физического действия на ID; E-stop удерживает фактический угол, reset ждёт подтверждённую парковку |
...
wall time и sim time иначе даёт правдоподобную, но неверную команду. Реальный
steady-state результат по пяти успешным потокам: медиана 18 товаров/мин,
p95-медленный такт 11 товаров/мин; камера→решение 0,094 с (p95 0,186 с),
решение→команда 1,90 с (p95 2,27 с). Первая задержка измеряется напрямую по
общему sim-clock кадра и решения, а не разностью строк из разных процессов.
...
- **E-stop:** отменяет таймеры и останавливает ленту. Скоростной толкатель
  замораживается нулевой скоростью; позиционный шибер получает измеренный
  `JointState`, а не прежнюю цель. После ручной очистки reset сначала паркует
  активные зоны и ждёт свежий угол в допуске 0.015 рад; повторный E-stop во
  время возврата снова удерживает измеренную позицию.
