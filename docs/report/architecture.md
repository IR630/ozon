...
wall time и sim time иначе даёт правдоподобную, но неверную команду. Принятый
steady-state результат полной потоковой суиты — `33/33`, 6/6 успешных эпизодов
и медиана `10 товаров/мин` при диапазоне эпизодов `4–15`; камера→решение —
`0,030 с` по общему sim-clock. Результат `18 товаров/мин` относится только к
короткому пиковому потоку и не заявляется как sustained-производительность.
...
- **Feedback-gated recovery — кандидат Stage 26:** при свежем `JointState`,
...
два feedback-топика кандидата Stage 26 требуют отдельного ресурсного и
latency-gate на точном окружении организаторов; неподдерживаемый throttle в SDF
не используется.
...
Три исторических pre-v0.5 body-scored census дали `33/33 ×3`. Финальный gate
принятой `v0.5-stable-stream` отдельно включает два полных stream-suite
`33/33 + 33/33`, а его census-цепочка состоит из префикса `32/32` и
возобновлённого хвоста Helmet `3/3`: их объединение покрывает 33 уникальные
ячейки, но это не один непрерывный прогон. Ограничение честности: seed фиксирует
сцену и ориентации, но контактная физика Gazebo имеет небольшой шум; поэтому
ревизии не смешиваются в один знаменатель, а каждый результат хранит собственный
каталог и triage.
