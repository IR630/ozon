...
не является одним непрерывным прогоном; непрерывную перепись `33/33` без
таймаутов и execution-ошибок даёт отдельный прогон на актуальном ядре
«v0.5 + Stage 26 + depth-scale fix». **Все эти числа сняты на одном seed'е.**
...
(`docs/report/path_to_line.md`). Более ранний multi-seed замер до depth-scale fix
давал диапазон `162–165/165` — тот же механизм отказа на другом ядре. Устойчивая
...
| `docs/report/methodology_and_limitations.md` | Как проверяли решение и чего сознательно не сделали — ядро честности отчёта |
| `docs/report/cameras.md` | Развилка числа камер: четыре построенные конфигурации и почему на оценку идёт одна головка |
| `docs/report/rig_decision.md` | Решение по стойке на линию: сколько головок, цена камеры против цены ошибки, что переворачивает вывод |
| `docs/report/path_to_line.md` | 16 режимов отказа реальной линии, которых в симуляторе нет в принципе |
| `docs/report/industry_comparison.md` | Сверка с промышленной практикой: сколько ракурсов ставят в реальных сортцентрах |
| `docs/report/slides/` | Четыре HTML-варианта деки защиты (9 слайдов, содержание одно) + `README.md` с запуском |
| `docs/defense/` | Материалы защиты: CAD-эскиз ячейки (`cad/`), `qa_prep.md`, вердикты LLM-совета `council_*.md`, `physics_oneslide.md` |
