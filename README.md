...
**Текущий результат.** Три полных body-scored census дали `33/33 ×3`. Устойчивая
производительность на **полной матрице всех 11 моделей** (потоковая суита `33/33`
доставлено, 6/6 эпизодов) — медиана `10 товаров/мин` (диапазон по эпизодам
`4–15`); пиковый короткий 3-товарный поток даёт `18–21`, но это не sustained-режим
и как заявка не используется. Startup Gazebo и soft-start ленты в метрику не входят
(T0 = полная скорость ленты). Замер: `measure_throughput.py` по
`runs/stream_suite_*`.
