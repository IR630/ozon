...
flowchart LR
    ...
    rig --> P["Перцепция<br/>item_id, габариты, K"]
    P --> C{"Классификатор<br/>габариты, затем форма"}

    C -->|"вне 10…450×320×320"| ZC["Зона C<br/>негабарит"]
    C -->|"K &gt; 0,8"| ZD["Зона D<br/>доупаковка"]
    C -->|"в габарите, не круг"| ZB["Зона B<br/>основной сортировщик"]

    C --> CTRL["Контроллер<br/>упреждение 0,5 м пути"]
    CTRL --> BL["Шибер<br/>стенка, склиз, возврат"]
    BL -.-> ZC
    BL -.-> ZD

    classDef zb fill:#06CA9A,stroke:#06CA9A,color:#04155E
    classDef zc fill:#FBE671,stroke:#FBE671,color:#04155E
    classDef zd fill:#F91155,stroke:#F91155,color:#fff
    class ZB zb
    class ZC zc
    class ZD zd
```

Товар едет по участку 5,7 с, а от кадра до вердикта проходит 0,042 с — решение
готово задолго до того, как товар доедет до механизма.
...
![Изометрия ячейки](docs/defense/cad/out/cell_sideview_iso.png)
