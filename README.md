...
демонстрации. Поверх неё принят один пакет: feedback-gated восстановление после
E-stop (Stage 26, ветка `int/estop-engaged-recovery`) **влито 21.07** после
полного регрессионного прогона. Физический occupied-smoke прошёл в GitHub
Actions run `29591607813` и с тех пор входит в обязательный `e2e-smoke`.
Открытым остаётся ресурсный/latency-gate двух новых feedback-топиков на точном
окружении организаторов.
