...
в Nextcloud уходит полный комплект.

**Ролик переписи — прямо в репозитории:** `docs/report/video/census_reel.mp4`,
**6:03**, 1280×720, H.264, 13,6 МБ. Это отдельная демонстрация РАБОТЫ решения без
разбора самого решения: 11 товаров × 3 сеяные ориентации, по клипу на ячейку,
каждый подписан товаром, позой, ожидаемой зоной и вердиктом переписи, счётчик
доходит до **33/33**. Снят на отгружаемой двухкамерной стойке
...
бокового. Пересобирается `python scripts/build_census_reel.py`. Он **не** заменяет
`demo_reel.mp4`: тот объясняет решение, этот показывает, что оно работает.
