...
**Необязательное питч-видео защиты — `docs/report/video/defence.mp4`:** 5:17, 1920×1080,
...
Пять клипов, которые играет слайд 8, лежат там же (`docs/report/video/`, H.264,
...
слайды, пробел — пуск клипа), структура и хронометраж —
`docs/report/presentation_outline.md`, готовый текст — `docs/report/defence_speech.md`.
...
```bash
python scripts/make_nextcloud_bundle.py     # -> build/II_v_massy_Task3/ + MANIFEST.md
# если финальные runs ещё у участника, выполнявшего прогоны:
python scripts/make_nextcloud_bundle.py --without-runs
```

В Nextcloud загружается **распакованная папка**, не ZIP/RAR. Режим
`--without-runs` явно отмечает отсутствие логов в `MANIFEST.md`; участник может
дозагрузить их позднее отдельной папкой `04-run-artifacts` до дедлайна.
...
| `04-run-artifacts` | дополнительные доказательные материалы | логи прогонов, из которых пересчитываются заявленные цифры; раздел можно дозагрузить позднее |
