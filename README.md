...
**163/165** — камера маршрутизацию не двигает, поэтому в сдаваемом ядре остаётся
одна верхняя RGBD как провалидированная сквозным контуром конфигурация.
...
**31/33 для двух камер при 32/33 для одной** (`docs/cameras-under-noise-brief.md`).
Исторический run-to-run диапазон прежних
...
| `docs/report/camera_tradeoff.md` | Измеренный выбор одной камеры для сдачи и gate пересмотра решения |
