...
# Production is the TWO-head rig (30.07); other rigs are one variable away.
...
#
# SOFT_START_TRIES x 0.5 s, same knob and same default as run_skeleton.sh — the two
# MUST stay equal, and tests/test_census_2v3_noise.py fails if they drift. It used
...
# Raised 60 -> 240 with run_skeleton.sh on 30.07 (shipped rig bring-up 28.7 s left
# a 1.3 s margin). This path matters more than the census one: it measures
# THROUGHPUT, the number the jury scores directly.
