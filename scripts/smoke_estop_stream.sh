...
DELIVERED=0
# 300 итераций (150 с), не 80 (40 с). Ожидание зашито в НАСТЕННЫХ секундах, а
# транзит товара по ленте занимает 5.7 с СИМУЛЯЦИОННЫХ, поэтому 40 с хватает
# только при RTF выше 0.14. На загруженном раннере CI (02.08, прогон
# 30732147879) товар за 40 с прошёл от -1.5 до 0.43, то есть 1.93 м = RTF 0.05,
# и смоук сообщил "post-reset item never reached terminal B" при полностью
# исправном контуре: y на выходе -0.000007, товар шёл ровно по центру, лопасть
# его не трогала. Тот же класс отказа, что уже чинили подъёмом SOFT_START_TRIES
# и CELL_TIMEOUT: бюджет в настенных секундах против работы в симуляционных.
# Цикл выходит по факту доставки, поэтому на здоровой машине подъём не стоит
# ничего.
