...
SEGMENTS: tuple[Card | Clip | Still, ...] = (
    ...
    Card((
        ...
        "",
        "Отгружаемая стойка: верхняя головка + два встречных борта",
    ), 7.0),
    Clip("rig3_bottle_D.mp4", "Бутылка → D: круг в сечении, K = 0,996"),
    Clip("rig3_box_300x200x200_B.mp4", "Короб 300×200×200 → B: в габарите, идёт как есть"),
    Clip("rig3_box_400x400x300_C.mp4", "Короб 400×400×300 → C: 400 мм против ворот 320"),
    Clip("rig3_pen_C.mp4", "Ручка → C: 9 мм — габарит бьёт форму (крупный план)"),
    ...
    Card((
        ...
        "пропускная способность отгружаемой стойки — 8 товаров/мин",
        "камера → решение — медиана 0,030 с",
    ...
    Card((
        "Честные границы",
        "",
        "8 товаров/мин — устойчивый режим, не пик; 18 — короткий пиковый поток.",
        "Плотный поток в одну зону деградирует: насыщается ход ленты.",
